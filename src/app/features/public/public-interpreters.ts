          ...
          <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
             <div>
                ...
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-base">
                   Connect with {{ totalElements() }} verified medical interpreters ready for consultation.
                </p>
             </div>
          </div>
...
export class FindInterpreterComponent implements OnInit {
   ...
   fetchPublicInterpreters() {
      ...
      // Using default pagination params for the public landing view
   ...
   // LOGIC: Bridge Public -> Private with Return URL
   handleViewProfile(interpreterId: number) {
      // The path to the protected details page
      const targetUrl = `/interpreters/${interpreterId}`;

      if (this.auth.isLoggedIn()) {
         // 1. If logged in, go straight to the protected page
         this.router.navigate([targetUrl]);
      } else {
         // 2. If NOT logged in, go to Login with a returnUrl
