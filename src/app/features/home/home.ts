                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden transform transition-all duration-500 hover:scale-[1.03] hover:shadow-2xl">
                      ...
                      <div class="p-4 space-y-3">
                        <div class="flex gap-4 p-3 rounded-lg border border-violet-200 dark:border-violet-800 bg-violet-50/50 dark:bg-violet-900/20">
                          <div class="w-10 h-10 rounded-full bg-cover bg-center flex-shrink-0" style="background-image:url('assets/home/lady_vcall.jpg')"></div>
                          <div class="flex-1">
  ...
  ngOnInit() {
    if (this.auth.isLoggedIn()) {
      ...
      if (user) {
        ...
        // Fallback: User logged in but state inconsistent (user obj null).
        // Show home page instead of blocking UI with isRedirecting=true
        this.isRedirecting = false;
      }
    }
  }

  // "Get Started" usually involves logic checking (auth vs no-auth), so we keep this handler
  handleGetStarted() {
    if (!this.auth.isLoggedIn()) {
    ...
    // Safety check: Ensure user data exists before redirecting
    const user = this.auth.currentUser();
    if (user) {
      this.navigateToDashboard(user);
    } else {
      // Edge case: Logged in but no user data found (token expired or store cleared)
      this.router.navigate(['/auth/login']);
    }
  }

  // Refactored to accept user object directly to avoid calling currentUser() repeatedly
