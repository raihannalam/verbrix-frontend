...
export class SafeVideoUrlPipe implements PipeTransform {
  ...
  transform(url: string | undefined): SafeResourceUrl {
    ...
    // Remove existing query params to avoid duplication issues
    ...
    try {
      if (cleanUrl.includes('youtube.com/watch')) {
        ...
        // Extract ID from vimeo.com/123456
...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gray-50 dark:bg-[#0b0c0f] font-sans transition-colors duration-200">
      ...
      @else if (data(); as interpreter) {
        <header class="bg-white dark:bg-[#181a1f] border-b border-gray-200 dark:border-gray-800 sticky top-0 z-30 shadow-sm backdrop-blur-md bg-opacity-95 supports-[backdrop-filter]:bg-opacity-80">
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center gap-4">
              <a routerLink="/dashboard/admin/home" 
                 class="p-2 -ml-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-all">
                <i class="ri-arrow-left-line text-xl"></i> 
              </a>
              
              <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                <span class="text-xs sm:text-sm text-gray-500 font-mono">#{{ interpreter.id }}</span>
            ...
            <div class="lg:col-span-4 space-y-6">
              <div class="lg:sticky lg:top-24 space-y-6">
                ...
                <div class="bg-white dark:bg-[#181a1f] rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-800 relative overflow-hidden">
                  <div class="absolute top-0 left-0 w-full h-24 bg-gradient-to-br from-blue-600 to-indigo-700 opacity-10 dark:opacity-20"></div>
                  <div class="relative flex flex-col items-center">
                    <div class="relative">
                      <img [src]="interpreter.profilePictureUrl || 'assets/placeholder.png'" 
                           class="w-28 h-28 rounded-full object-cover border-4 border-white dark:border-[#20232a] shadow-lg mb-4 bg-gray-100 dark:bg-gray-800">
                      @if(interpreter.status === 'VERIFIED') {
                    ...
                    <div class="w-full grid grid-cols-2 gap-3">
                      ...
                      <div class="p-3 bg-gray-50 dark:bg-[#20232a] rounded-xl text-center border border-gray-100 dark:border-gray-800">
                        ...
                        <div class="text-lg font-bold text-green-600 dark:text-green-400">
                          {{ interpreter.consultationFee | currency }}<span class="text-xs font-normal text-gray-500">/min</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <a [href]="interpreter.governmentIdUrl" target="_blank" 
                   class="flex items-center gap-4 p-4 bg-white dark:bg-[#181a1f] rounded-xl border border-blue-100 dark:border-blue-900/30 hover:shadow-md transition-all group">
                   <div class="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 flex items-center justify-center group-hover:scale-110 transition-transform">
                     <i class="ri-passport-line text-xl"></i>
                   </div>
                   <div class="flex-1 min-w-0">
                     <div class="font-bold text-gray-900 dark:text-white text-sm truncate">Government ID</div>
                     <div class="text-xs text-blue-600 dark:text-blue-400 group-hover:underline">Click to view document</div>
                   </div>
                    ...
                    <div class="flex flex-col gap-2">
                      @for (lang of interpreter.languageAbilities; track lang.language) {
                        <div class="p-2.5 rounded-lg bg-gray-50 dark:bg-[#20232a] text-sm flex items-center justify-between border border-gray-100 dark:border-gray-800">
                          <span class="text-gray-900 dark:text-gray-100 font-bold">{{ getLanguageLabel(lang.language) }}</span>
      ...
      <div class="fixed bottom-0 left-0 right-0 p-4 bg-white/90 dark:bg-[#181a1f]/95 border-t border-gray-200 dark:border-gray-800 md:hidden z-40 backdrop-blur-md shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] flex gap-3 safe-area-pb">
        <button (click)="openActionModal('REJECT_APP')" class="flex-1 py-3 text-sm font-bold text-red-600 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-xl">Reject</button>
        <button (click)="openActionModal('REQUEST_CHANGES')" class="flex-1 py-3 text-sm font-bold text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl">Changes</button>
        <button (click)="openActionModal('APPROVE_APP')" [disabled]="!allCertsVerified()" class="flex-[1.5] py-3 text-sm font-bold text-white bg-green-600 dark:bg-green-700 rounded-xl shadow-lg disabled:opacity-50 disabled:grayscale">Approve</button>
      </div>
             ...
             <div class="p-6 overflow-y-auto">
                ...
                @if (actionModal().needsInput) {
                  <div class="space-y-2 mb-6">
                    ...
                    <textarea [(ngModel)]="reasonText" 
                              rows="4" 
                              class="w-full p-4 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#121418] text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none shadow-sm placeholder:text-gray-400" 
                ...
                <div class="flex gap-3">
                   <button (click)="closeModal()" 
                           class="flex-1 py-3 px-4 font-bold text-sm text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl transition-colors">
                     Cancel
                   </button>
                   <button (click)="confirmAction()" 
                           [disabled]="isSubmitting() || (actionModal().needsInput && !reasonText().trim())"
                           class="flex-1 py-3 px-4 font-bold text-sm text-white rounded-xl shadow-lg flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                           [ngClass]="{
  ...
  confirmAction() {
    ...
    if (action$) {
      action$.pipe(
        ...
        next: () => {
          this.closeModal();
          // Logic: If it was a page-level status change, go home. 
          // If it was just a cert update, reload data.
